...
function isAdmitted(gpa, satScore, recommendation) {
    if (gpa > 4 || satScore > 1300 || gpa > 3 && recommendation == true
 ...
 * @param {string} [] 
 * @returns {string} [] updated array with 'Happy '
 */
function makeHappy(array) {
    let semanticName = array.map((e) => 'Happy ' + e)
    return semanticName;
};
...
function getFullAddressesOfProperties(arrOfObjects) {
    ...
    return arrOfObjects.map((p) => {
        return [p.streetNumber, p.streetName, p.streetType, p.city, p.state, p.zip].join(' ');
    });

}
    // let newArray = arrOfObjects.map((obj) => { return [obj.streetNumber + " ", obj.streetName + " ", obj.streetType + " ", obj.city + " ", obj.state + " ", obj.zip + " "] });
    // return newArray;
